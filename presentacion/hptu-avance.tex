...
El HPTU opera hoy en Robledo con \highlight{540 camas}.
...
\small HPTU Robledo: \textbf{540 camas}.\\
Capacidad actual del hospital.
\end{alertblock}
\end{columns}
\end{frame}
...
\midrule
HPTU Robledo & \cellcolor{teal!10}\highlight{23.8} & 31.8 & \danger{57.1} & 22.4 & \highlight{19.0} \\
...
A \textbf{23.8 min} de HPTU Robledo actual.\\[4pt]
...
\small \danger{0 camas} de alta complejidad en el corredor.\\
Cl.\ Rosario Tesoro (158) y Cl.\ Las Vegas (171) son las más cercanas, sin trasplantes.\\[4pt]
Clínica CES (213 camas) está en \textbf{Prado Centro} (7.16 km del corredor).\\[4pt]
...
\vspace{6pt}
Con la red hospitalaria con solo \textbf{1,593 camas}, el sistema no tiene cómo absorber ese crecimiento sin nueva infraestructura.
\end{columns}
\end{frame}
...
\node[pillar] (p4) at (9.6,0) {
  ...
  A HPTU Robledo\\{\scriptsize Mapbox Matrix}\\10.5' a\\Cl. Las Vegas
};
...
% TRÁFICO VALIDADO
% ============================================================
\begin{frame}{Tráfico validado: 8 tramos con Mapbox + Google Routes}

\begin{columns}[T]
\column{0.55\textwidth}
\textbf{6 tramos Las Palmas + 2 tramos El Poblado}, datos en tiempo real:

\vspace{6pt}
{\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Tramo} & \textbf{km/h} & \textbf{Nivel} \\
\midrule
San Diego → Intercont. & \danger{16.3} & Severo \\
Intercont. → Túnel & 15.3 & Severo \\
\rowcolor{coral!12}
\textbf{Túnel → Colegios} & \danger{13.4} & \danger{Crítico} \\
Colegios → Los Balsos & \highlight{30.7} & Fluido \\
Los Balsos → Colegiatura & 25.0 & Moderado \\
Colegiatura → Alto & 24.2 & Moderado \\
\midrule
Pq.\ Poblado → Oviedo & 21.1 & Moderado \\
\rowcolor{coral!8}
Oviedo → El Tesoro & 20.5 & \danger{Heavy} \\
\bottomrule
\end{tabular}
}

\src{Mapbox Directions API + Google Routes API, marzo 2026}

\column{0.42\textwidth}
\begin{alertblock}{\small Cuello de botella identificado}
\small Segmento 3: \textbf{Túnel → Zona Colegios}

\vspace{4pt}
\begin{itemize}\small
  \item \danger{13.4 km/h} promedio (Mapbox)
  \item Velocidades mínimas de \textbf{5 km/h}
  \item Zona de colegios Columbus/Carabobo
  \item 4.7 km en 21 min
\end{itemize}
\end{alertblock}

\vspace{6pt}
\begin{exampleblock}{\small Validación cruzada}
\small Mapbox y Google coinciden: la zona baja de Las Palmas (San Diego--Colegios) tiene congestión comparable a El Poblado.\\[4pt]
\textbf{La zona candidata Las Palmas Bajo} está \emph{después} de este cuello de botella.
\end{exampleblock}
\end{columns}
\end{frame}

% ============================================================
% ACCESIBILIDAD DESDE 6 ORÍGENES
% ============================================================
\begin{frame}{Accesibilidad: tiempos reales desde 6 orígenes}

Destino: \textbf{Las Palmas Bajo} (6.195, -75.549) — mejor tiempo de cada origen:
...
\begin{tabular}{@{}lcccl@{}}
\toprule
\textbf{Origen} & \textbf{Tiempo} & \textbf{Distancia} & \textbf{Fuente} & \textbf{Mejor ruta} \\
\midrule
\rowcolor{teal!10}
Milla de Oro & \highlight{15.4 min} & 5.1 km & Google & Cra.\ 17 \\
\rowcolor{teal!8}
El Poblado (Parque) & \highlight{17.7 min} & 5.4 km & Google & Cra.\ 29 \\
Envigado Centro & 24.8 min & 7.6 km & Google & Tv.\ Superior \\
Laureles & 32.3 min & 11.6 km & Mapbox & Ruta 25, Cl.\ 5 Sur \\
Itagüí Centro & 33.4 min & 11.0 km & Mapbox & Av.\ Las Vegas \\
\rowcolor{coral!8}
San Diego / Centro & \danger{36.3 min} & 11.2 km & Mapbox & Av.\ Las Palmas \\
\midrule
...
\vspace{6pt}
\src{Mapbox Directions API (driving-traffic) + Google Routes API (TRAFFIC\_AWARE), marzo 2026}

\vspace{8pt}
\begin{columns}[T]
\column{0.48\textwidth}
\begin{exampleblock}{\small Mercado premium accesible}
\small El \textbf{80\% del mercado E5/E6} (Milla de Oro, El Poblado, Envigado) llega en \highlight{menos de 25 min}.
\end{exampleblock}

\column{0.48\textwidth}
\begin{block}{\small Corredores clave}
\small Cra.\ 17 / Tv.\ Superior evitan la congestión de Las Palmas inferior. La Cl.\ 5 Sur es alternativa eficiente desde el sur del valle.
\end{block}
\end{columns}
\end{frame}

% ============================================================
% HALLAZGO: 0 CAMAS ALTA COMPLEJIDAD
% ============================================================
{
\setbeamercolor{background canvas}{bg=coral!3}
\begin{frame}{Hallazgo: 0 camas de alta complejidad en el corredor}

\begin{columns}[T]
\column{0.52\textwidth}
La Clínica CES (\textbf{213 camas}) fue inicialmente reportada ``en la vía Las Palmas''.

\vspace{6pt}
Verificación con \textbf{Google Geocoding API}:

\vspace{6pt}
\begin{alertblock}{\small Corrección}
\small La Clínica CES está en \textbf{Prado Centro}\\
Calle 52 No.\ 46-48, Medellín\\
Coordenadas: 6.2573, -75.5655\\[4pt]
\danger{A 7.16 km del corredor Las Palmas}\\
Tiempo de viaje: \textbf{25+ min} en hora pico
\end{alertblock}

\vspace{6pt}
Esto significa que \textbf{el corredor Las Palmas tiene exactamente \danger{0 camas} de alta complejidad}.

\column{0.45\textwidth}
\textbf{Hospitales más cercanos al corredor:}

\vspace{6pt}
{\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Hospital} & \textbf{Camas} & \textbf{km} \\
\midrule
Cl.\ Rosario Tesoro & 158 & 2.1 \\
Cl.\ Las Vegas & 171 & 2.8 \\
Cl.\ Medellín Poblado & 85 & 3.2 \\
Cl.\ del Country & 62 & 3.5 \\
\rowcolor{coral!8}
Cl.\ CES & 213 & \danger{7.16} \\
\rowcolor{coral!8}
HPTU Robledo & 540 & \danger{8.5} \\
\bottomrule
\end{tabular}
}

\vspace{8pt}
\begin{exampleblock}{\small Implicación}
\small Ninguno de los hospitales cercanos al corredor ofrece \textbf{trasplantes ni hematología}. El HPTU sería el \textbf{único referente de alta complejidad} en la zona.
\end{exampleblock}
\end{columns}
\end{frame}
}
...
% ============================================================
\begin{frame}{Modelo financiero preliminar: factibilidad por zona}
...
\midrule
\rowcolor{teal!10} Las Palmas Bajo & 250 & 90MM & 11.3y & +4.8MM \\
Las Palmas Medio & 300 & 108MM & 9.4y & +15.3MM \\
Envigado--Zúñiga & 220 & 82MM & 11.3y & +4.0MM \\
Nuevo Poblado & 260 & 98MM & 9.6y & +14.7MM \\
Las Palmas Alto & 150 & 55MM & 22.9y & --31.0MM \\
\bottomrule
\end{tabular}
}

\vspace{6pt}
{\tiny Parámetros base: Construcción \$4.0M COP/m², dotación 25\%, ocupación 80\%, ingreso \$1,600M/cama/año, EBITDA 8\%, WACC 12\%.\\
VPN con ramp-up 60\%--80\%--100\% en años 1--2--3+. \textcolor{coral}{\textbf{Cifras en miles de millones COP.}}}

\column{0.40\textwidth}
\begin{alertblock}{\small Parámetros por validar}
\tiny
\begin{itemize}
  \item[\textcolor{coral}{$\blacktriangledown$}] Precio suelo/m² por zona (Lonja de Propiedad Raíz)
  \item[\textcolor{coral}{$\blacktriangledown$}] Ingreso/cama/año alta complejidad privada
  \item[\textcolor{coral}{$\blacktriangledown$}] Margen EBITDA operativo real
  \item[\textcolor{amber}{$\blacktriangledown$}] WACC específico del proyecto
  \item[\textcolor{coral}{$\blacktriangledown$}] Fichas normativas POT por polígono
  \item[\textcolor{coral}{$\blacktriangledown$}] Tamaño lote disponible por zona
\end{itemize}
\end{alertblock}

\begin{block}{\small Fuentes}
\tiny
Construcción: Scielo/Univ. Militar. Benchmark: HUSI 2024.\\
Suelo: Fincaraíz, Metrocuadrado (Mar 2026).\\
Población: DANE CNPV 2018 + proyecciones.
\end{block}
\end{columns}
...
\vspace{20pt}
{\footnotesize\textcolor{white!50}{4 de marzo, 2026}}
\end{center}
\vfill
\end{frame}
}
